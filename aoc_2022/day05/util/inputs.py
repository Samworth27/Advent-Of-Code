...
def fields(line:str, positions:list[int], map_function = lambda x:x) -> list:
    return [map_function(field) for position, field in enumerate(line.split()) if position in positions]
